...
The scale to handle Celcius is a bit of a kludge, but works
...
def Show_Temp_Ticks():
    ...
    while T_Mark > 6:
        ...
        display.text(S_Temp, (R + 5), (T_Mark - 5), scale=2)       #Print the text, offset 5 pixels up from line
        display.update()
        Temp += 2
    ...
    while T_Mark > 6:
        ...
        display.text(S_Temp, 3, (T_Mark - 5), scale=2)       #Print the text, offset 5 pixels up from line
        display.update()
        Temp += 2
        T_Mark -= 18                                          #Not enough room so only every 2 degrees
