# ON-OFF Slide Switch - Simple Polling using compare
...
Slide_Sw = machine.Pin(15,machine.Pin.IN,machine.Pin.PULL_DOWN)

#Polling method using compare to handle an ON-OFF switch
print("Ready, Set, GO!")
while True:  #run an endless loop - Typical main loop
    if Slide_Sw.value() == True: #If input GP15 is HIGH
        LED.value(1)
        print("ON")
    elif Slide_Sw.value() == False: #If input GP15 is LOW
        print("OFF")  
        LED.value(0)
