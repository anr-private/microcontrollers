'''
Fonts for the WaveShare 1.3" IPS LCD Display Module for Raspberry Pi Pico (240x240)
                         Tony Goodhew 16th Aug 2021
...
Modified after a suggestion from Steve Borg:
"Refactor it slightly so that the various functions take an optional parameter to tell it
whether to do a LCD.show() you can get it so it draws much more quickly.
...
printchar now has the extra parameter: Charupdate (Boolean (True/False)
...
These say whether to call LCD.show()on individual chars (slow) or on the entire string (fast)
You can set them both to false if you want, send multiple lines, then do a LCD.update() manually if you like
...
c = colour(255,0,0)    # c holds the colour to be used
printstring("!! Hello World !!",0,0,2,True,False,c) # Slow
c = colour(0,255,0)
printstring("Les Wright 2021 - Tiny font",9,30,1,False,True,c) # Fast string

c = colour(0,0,255)
printstring('abcdefghijk',0,60,2,True,False,c) # one char at a time, like an old serial terminal
printstring('lmnopqrstuvwxyz',0,80,2,True,False,c) # slow
printstring('ABCDEFGHIJKLM',0,100,2,True,False,c)
printstring('NOPQRSTUVWXYZ',0,120,2,True,False,c)

printstring('`~!@#$%^&*()_-+=[',0,140,2,False,False,c) # Fast
printstring(']\\{}|;\':"<>?,./',0,160,2,False,False,c)
LCD.show()

c = colour(255,255,0) 
printstring("Text size 3",17,190,3,0,0,c) # Easier format
c = colour(255,255,255) # White
printstring("Text size 1",17,220,1,0,0,c) # Tiny Les Wright font
LCD.text("Framebuf Text", 110, 220, colour(0,255,255)) # Built in framebuf tiny fount
LCD.show()
spinner = '-\|/-\|/'
for i in range(30):
    for x in spinner:
        c=colour(255,255,0)
        printchar(x,190,50,2,True,c)
        delchar(190,50,2,True)
