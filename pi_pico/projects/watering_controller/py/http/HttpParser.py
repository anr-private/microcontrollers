...
class HttpParser:
    ...
    def parse_header_data(self, hdr_data):
        ...
        dbg(f"HP@30 parse_header_data  >>>>  {"NO-DATA" if hdr_data is None else len(hdr_data)}")
        ...
        if hdr_data.find(eol_sep) < 0:
            ...
            if hdr_data.find(eol_sep) < 0:
                ...
                dbg(f"HP@44 parse_header_data {self._latest_error}")
                return None
        dbg(f"HP@46 parse_header_data expected eol: {repr(eol_sep)}")
        ...
        dbg(f"HP@53 parse_header_data SPLIT LINES {len(lines)=}  ")

        if len(lines) < 3:
            ###dbg(f"HP@56 parse_header_data ONLY {len(lines)} HEADER LINE(S) FOUND after splitting on eol {repr(eol_sep)}")
            self.err(f"ONLY {len(lines)} HEADER LINE(S) FOUND after splitting on eol {repr(eol_sep)}")
            dbg(f"HP@58 parse_header_data {self._latest_error}")
            return None

        dbg(f"HP@61 lines are {lines}")
        ...
        if len(second_to_last_line) != 0:
            ...
            return None
        dbg(f"HP@73 found empty line end-of-hdr")
        ...
        dbg(f"HP@81 parse_start_line  {ok=}")
        if not ok: 
    ...
    def parse_start_line(self, ph, start_line):
        ...
        """
        ###dbg(f"HP@103  {start_line=}  ph={ph}")
        dbg(f"HP@104  {start_line=}  ph={ph}")
    ...
    def parse_request_start_line(self, ph, start_line):
        """ Parse the start line of a Request. ie the first line:
            Request ex: GET / HTTP/1.1
                        POST /api/users HTTP/1.1
        """
        dbg(f"HP@123 {start_line=}")

        parts = start_line.split()
        dbg(f"HP@126 parts={parts}")
        if len(parts) < 3:
        ...
        dbg(f"HP@134 {method_stg=}  {request_url=} {version_stg_val=}  ")
    ...
    def parse_reply_start_line(self, ph, start_line):
        ...
        """
        parts = start_line.split()
        dbg(f"HP@165 parse_reply_start_line parts={parts}")
        if len(parts) < 3:
        ...
        dbg(f"HP@173 {version_stg_val=}  {numeric_code_val=}  {reply_stg_val=}  ")

        got_version = self.parse_http_version(version_stg_val)

        dbg(f"HP@177  {got_version=}")
        if not got_version:
            return None
    ...
    def parse_url(self, full_url):
        ...
        dbg(f"HP@251 parts[#]  {parts}")
        frag = ""
        if len(parts) > 1:
            frag = parts[1]
            url_stg = parts[0]
        dbg(f"HP@256  {frag=}  {url_stg=}")

        parts = url_stg.rsplit("?", 1)
        dbg(f"HP@259 parts[?]  {parts}")
        ...
        if len(parts) > 1:
            ...
            path = url_stg
            query = ""
        dbg(f"HP@267  {path=}  {query=}")

        query_parts = query.split("&")
        dbg(f"HP@270 parts-of-query  {query_parts}")

        query_params = {}
        for qitem in query_parts:
            dbg(f"HP@274 {qitem=}")
            parts = qitem.split("=",1)
            dbg(f"HP@276 Qparts {parts}")
            qkey = parts[0]
            qval = ""
            if len(parts) > 1:
                qval = parts[1]
            query_params[qkey] = qval
        dbg(f"HP@282 {query_params}")
