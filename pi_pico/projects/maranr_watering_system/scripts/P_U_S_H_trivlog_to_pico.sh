...
copy_release_files_to_the_pico() {
    echo 'Copy contents of trivlog/ '
    mpremote ${PREM_DEVICE} fs cp -r trivlog   :
...
list_pico_filesystem_contents() {
    ...
    echo ' '    
    echo 'LIST THE PICO CONTENTS: /trivlog'
    mpremote ${PREM_DEVICE} fs ls /trivlog
}                                                             


main() {
    echo " PUSH_trivlog_to_pico -- MAIN()"
    clean_up_old_dirs_and_files
    copy_release_files_to_the_pico
    list_pico_filesystem_contents
}
