...
copy_release_files_to_the_pico() {
    ...
    mpremote ${PREM_DEVICE} fs cp -r test_files   :
...
list_pico_filesystem_contents() {
    ...
    echo ' '    
    echo 'LIST THE PICO CONTENTS: /test_files'
    mpremote ${PREM_DEVICE} fs ls /test_files
}


main() {
    echo "PUSH_LIB_FILES_TO_PICO"
    clean_up_old_dirs_and_files
    copy_release_files_to_the_pico
    list_pico_filesystem_contents
}
