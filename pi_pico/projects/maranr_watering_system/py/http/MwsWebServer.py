...
class MwsWebServer:
    ...
    async def webserver_coro(self):
        ...
        dbg(f"MWS@29 webserver_coro: Listening on {self.host}:{self.port}...")
        dbg(f"MWS@30  Server obj is {type(server)}")

        while 1:
            dbg(f"MWS@33 webserver_coro RUNNING idle!")
            await asyncio.sleep(5)

        ###result = "NO RESULT YET from webserver_coro"
        ###dbg(f"MWS@37 webserver_coro COMPLETED.  {result=}")
        ###return result
    ...
    async def handle_new_client(self, reader, writer):
        ...
        try:
            while 1:
                dbg(f"MWS@54 handle_new_client@54  ======  READ A LINE  =================================")
                ...
                if hdrAccum.found_end_of_header():
                    dbg(f"MWS@65 >>>--->>>  found EOHdr   hdrlen={len(hdrAccum.get_header())}")
                    header = hdrAccum.get_header()
                    mesg_tail = hdrAccum.get_tail()
                    dbg(f"MWS@68 {header if header else 'NULL'}")
                    dbg(f"MWS@69 {mesg_tail if mesg_tail else 'NULL'}")
        ...
        dbg("MWS@104 handle_new_client CLIENT WRITER is CLOSED")
        
        dbg(f"MWS@106 handle_new_client CLOSING THE CLIENT reader")
        reader.close()
        await reader.wait_closed()
        dbg("MWS@109 handle_new_client CLIENT READER is CLOSED")


    def handle_the_request(self, header):
        dbg(f"MWS@113 handle_client_request  {len(header)=}")

        reqHandler = RequestHandler()
        httpReply = reqHandler.handle_client_request(header)

        dbg(f"MWS@118 httpReply: {str(httpReply)}")
        return httpReply
