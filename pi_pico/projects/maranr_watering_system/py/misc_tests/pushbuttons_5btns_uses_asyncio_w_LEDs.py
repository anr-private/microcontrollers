# pushbuttons_5btns_uses_asyncio_w_LEDs.py
#
...
async def handle_long(btn, btn_name):
    ...
    if btn_name == "Up":
        update_blinking(led06)
        update_blinking(led10)
    elif btn_name == "Left":
        update_blinking(led07)
    elif btn_name == "Right":
        update_blinking(led08)
    elif btn_name == "Down":
        update_blinking(led09)
        update_blinking(led11)
    elif btn_name == "Center":
        RUN_FLAG = False

def update_blinking(led):
    led.toggle()
    if led in leds_blinking:
        leds_blinking.remove(led)
    else:
        leds_blinking.add(led)

async def blinking_task():
    # cause leds to blink if in the leds_blinking set
    while 1:
        await asyncio.sleep(1)
        for led in leds_blinking:
            led.toggle()

async def OLD__task_1(): # fake task
    global TASK_1_CTR
    TASK_1_CTR = 0
    while 1:
        TASK_1_CTR += 1
...
async def main():
    ...
    if 1:
        print("MAIN@162  Starting blinking_task")
        asyncio.create_task(blinking_task())
