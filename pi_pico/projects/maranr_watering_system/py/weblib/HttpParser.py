...
class HttpParser(ElemLoggerABC):
    ...
    def parse_header_data(self, hdr_data):
        ...
        if hdr_data.find(eol_sep) < 0:
            ...
            if hdr_data.find(eol_sep) < 0:
                ...
                log(f"HP@57 parse_header_data {self._latest_error}")
                return None
        log(f"HP@59 parse_header_data expected eol: {repr(eol_sep)}")
        ...
        log(f"HP@66 parse_header_data SPLIT LINES {len(lines)=}  ")

        if len(lines) < 3:
            ###log(f"HP@69 parse_header_data ONLY {len(lines)} HEADER LINE(S) FOUND after splitting on eol {repr(eol_sep)}")
            self.err(f"ONLY {len(lines)} HEADER LINE(S) FOUND after splitting on eol {repr(eol_sep)}")
            log(f"HP@71 parse_header_data {self._latest_error}")
            return None

        log(f"HP@74 lines are {lines}")
        ...
        if len(second_to_last_line) != 0:
            ...
            return None
        log(f"HP@86 found empty line end-of-hdr")
        ...
        log(f"HP@94 parse_start_line  {ok=}")
        if not ok: 
    ...
    def parse_request_start_line(self, ph, start_line):
        ...
        log(f"HP@147 {method_stg=}  {request_url=} {version_stg_val=}  ")
        ...
        log(f"HP@155  {got_version=}")
        if not got_version:
            return None

        parsed_url = self.parse_url(request_url)
        log(f"HP@160  {parsed_url}")
        if not parsed_url:
    ...
    def parse_reply_start_line(self, ph, start_line):
        ...
        """
        parts = start_line.split()
        log(f"HP@178 parse_reply_start_line parts={parts}")
        if len(parts) < 3:
        ...
        log(f"HP@186 {version_stg_val=}  {numeric_code_val=}  {reply_stg_val=}  ")

        got_version = self.parse_http_version(version_stg_val)

        log(f"HP@190  {got_version=}")
        if not got_version:
            return None

        num_code_as_int = string_to_int(numeric_code_val)
        log(f"HP@195 {num_code_as_int=}")
    ...
    def parse_url(self, full_url):
        ...
        if len(parts) > 1:
            frag = parts[1]
            url_stg = parts[0]
        log(f"HP@269  {frag=}  {url_stg=}")

        parts = url_stg.rsplit("?", 1)
        log(f"HP@272 parts[?]  {parts}")
