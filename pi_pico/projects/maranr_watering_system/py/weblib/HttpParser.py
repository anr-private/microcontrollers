...
class HttpParser(ElemLoggerABC):
    ...
    def parse_header_data(self, hdr_data):
        ...
        log(f"HP@30 parse_header_data  >>>>  {"NO-DATA" if hdr_data is None else len(hdr_data)}")
        ...
        if hdr_data.find(eol_sep) < 0:
            ...
            if hdr_data.find(eol_sep) < 0:
                ...
                log(f"HP@44 parse_header_data {self._latest_error}")
                return None
        log(f"HP@46 parse_header_data expected eol: {repr(eol_sep)}")
        ...
        log(f"HP@53 parse_header_data SPLIT LINES {len(lines)=}  ")

        if len(lines) < 3:
            ###log(f"HP@56 parse_header_data ONLY {len(lines)} HEADER LINE(S) FOUND after splitting on eol {repr(eol_sep)}")
            self.err(f"ONLY {len(lines)} HEADER LINE(S) FOUND after splitting on eol {repr(eol_sep)}")
            log(f"HP@58 parse_header_data {self._latest_error}")
            return None

        log(f"HP@61 lines are {lines}")
        ...
        if len(second_to_last_line) != 0:
            ...
            return None
        log(f"HP@73 found empty line end-of-hdr")
        ...
        log(f"HP@81 parse_start_line  {ok=}")
        if not ok: 
    ...
    def parse_start_line(self, ph, start_line):
        ...
        """
        ###log(f"HP@103  {start_line=}  ph={ph}")
        log(f"HP@104  {start_line=}  ph={ph}")
    ...
    def parse_request_start_line(self, ph, start_line):
        """ Parse the start line of a Request. ie the first line:
            Request ex: GET / HTTP/1.1
                        POST /api/users HTTP/1.1
        """
        log(f"HP@123 {start_line=}")

        parts = start_line.split()
        log(f"HP@126 parts={parts}")
        if len(parts) < 3:
        ...
        log(f"HP@134 {method_stg=}  {request_url=} {version_stg_val=}  ")
        ...
        log(f"HP@142  {got_version=}")
        if not got_version:
            return None

        parsed_url = self.parse_url(request_url)
        log(f"HP@147  {parsed_url}")
        if not parsed_url:
    ...
    def parse_reply_start_line(self, ph, start_line):
        ...
        """
        parts = start_line.split()
        log(f"HP@165 parse_reply_start_line parts={parts}")
        if len(parts) < 3:
        ...
        log(f"HP@173 {version_stg_val=}  {numeric_code_val=}  {reply_stg_val=}  ")

        got_version = self.parse_http_version(version_stg_val)

        log(f"HP@177  {got_version=}")
        if not got_version:
            return None

        num_code_as_int = string_to_int(numeric_code_val)
        log(f"HP@182 {num_code_as_int=}")
    ...
    def parse_http_version(self, ver):
        ...
        http_part = parts[0]
        version = parts[1]
        log(f"HP.parse_http_version@175 http_part {http_part}  version {version}")
    ...
    def parse_url(self, full_url):
        ...
        log(f"HP@251 parts[#]  {parts}")
        frag = ""
        if len(parts) > 1:
            frag = parts[1]
            url_stg = parts[0]
        log(f"HP@256  {frag=}  {url_stg=}")

        parts = url_stg.rsplit("?", 1)
        log(f"HP@259 parts[?]  {parts}")
        ...
        if len(parts) > 1:
            ...
            path = url_stg
            query = ""
        log(f"HP@267  {path=}  {query=}")

        query_parts = query.split("&")
        log(f"HP@270 parts-of-query  {query_parts}")

        query_params = {}
        for qitem in query_parts:
            log(f"HP@274 {qitem=}")
            parts = qitem.split("=",1)
            log(f"HP@276 Qparts {parts}")
            qkey = parts[0]
            qval = ""
            if len(parts) > 1:
                qval = parts[1]
            query_params[qkey] = qval
        log(f"HP@282 {query_params}")
