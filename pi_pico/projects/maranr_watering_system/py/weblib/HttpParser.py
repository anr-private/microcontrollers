...
class HttpParser(ElemLoggerABC):
    ...
    def parse_header_data(self, hdr_data):
        ...
        log(f"HP@42 parse_header_data  >>>>  {'NO-DATA' if hdr_data is None else len(hdr_data)}")
        ...
        if hdr_data.find(eol_sep) < 0:
            ...
            if hdr_data.find(eol_sep) < 0:
                ...
                log(f"HP@56 parse_header_data {self._latest_error}")
                return None
        log(f"HP@58 parse_header_data expected eol: {repr(eol_sep)}")
        ...
        log(f"HP@65 parse_header_data SPLIT LINES {len(lines)=}  ")

        if len(lines) < 3:
            ###log(f"HP@68 parse_header_data ONLY {len(lines)} HEADER LINE(S) FOUND after splitting on eol {repr(eol_sep)}")
            self.err(f"ONLY {len(lines)} HEADER LINE(S) FOUND after splitting on eol {repr(eol_sep)}")
            log(f"HP@70 parse_header_data {self._latest_error}")
            return None

        log(f"HP@73 lines are {lines}")
        ...
        if len(second_to_last_line) != 0:
            ...
            return None
        log(f"HP@85 found empty line end-of-hdr")
        ...
        log(f"HP@93 parse_start_line  {ok=}")
        if not ok: 
    ...
    def parse_request_start_line(self, ph, start_line):
        ...
        log(f"HP@146 {method_stg=}  {request_url=} {version_stg_val=}  ")
        ...
        log(f"HP@154  {got_version=}")
        if not got_version:
            return None

        parsed_url = self.parse_url(request_url)
        log(f"HP@159  {parsed_url}")
        if not parsed_url:
    ...
    def parse_reply_start_line(self, ph, start_line):
        ...
        """
        parts = start_line.split()
        log(f"HP@184 parse_reply_start_line parts={parts}")
        if len(parts) < 3:
        ...
        log(f"HP@192 {version_stg_val=}  {numeric_code_val=}  {reply_stg_val=}  ")

        got_version = self.parse_http_version(version_stg_val)

        log(f"HP@196  {got_version=}")
        if not got_version:
            return None

        num_code_as_int = string_to_int(numeric_code_val)
        log(f"HP@201 {num_code_as_int=}")
    ...
    def parse_url(self, full_url):
        ...
        if len(parts) > 1:
            frag = parts[1]
            url_stg = parts[0]
        log(f"HP@276  {frag=}  {url_stg=}")

        parts = url_stg.rsplit("?", 1)
        log(f"HP@279 parts[?]  {parts}")
        ...
        if len(parts) > 1:
            ...
            path = url_stg
            query = ""
        log(f"HP@287  {path=}  {query=}")

        if query:
            query_parts = query.split("&")
            log(f"HP@291 parts-of-query  {query_parts}")
    
            query_params = {}
            for qitem in query_parts:
                log(f"HP@295 {qitem=}")
                parts = qitem.split("=",1)
                log(f"HP@297 Qparts {parts}")
                qkey = parts[0]
                qval = ""
                if len(parts) > 1:
                    qval = parts[1]
